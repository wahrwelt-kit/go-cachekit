...
// DefaultSieveCacheSize is the default maximum number of entries when NewSieveCache is called with maxSize <= 0.
const DefaultSieveCacheSize = 100
...
// SieveCache is an in-memory cache that uses a SIEVE-style second-chance eviction algorithm.
// It uses lazy promotion (visited bit set on access) combined with quick demotion
// (new entries are evicted quickly if never accessed again), making it a good fit for skewed
// workloads where a small hot set is mixed with one-off scans. All operations are O(1) amortized.
...
// NewSieveCache returns a SieveCache that holds at most maxSize entries. If maxSize <= 0, DefaultSieveCacheSize (100) is used.
func NewSieveCache[K comparable, V any](maxSize int) *SieveCache[K, V] {
	if maxSize <= 0 {
		maxSize = DefaultSieveCacheSize
	}
	return &SieveCache[K, V]{
		items:   make(map[K]*sieveNode[K, V], maxSize),
		maxSize: maxSize,
	}
}
...
func (c *SieveCache[K, V]) addToHead(n *sieveNode[K, V]) {
	n.prev = nil
	n.next = c.head
	if c.head != nil {
		c.head.prev = n
...
func (c *SieveCache[K, V]) removeNode(n *sieveNode[K, V]) {
	if n.prev != nil {
		n.prev.next = n.next
	} else {
		c.head = n.next
...
// and the entry is marked as visited. If the cache is full, the SIEVE-style eviction pass removes
// the first unvisited entry. No-op if the receiver is nil
...
func (c *SieveCache[K, V]) SetIfAbsent(key K, value V) {
	if c == nil {
		return
	}
	c.mu.Lock()
